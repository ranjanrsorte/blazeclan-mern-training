...
let group = createGroupedProductsList(ProductList, 'categoryName');
...
function getRadioButtons(groupby) {
    let groupByKeys = Object.keys(createGroupedProductsList(ProductList, groupby));
    let input = '';

    for (let u = 0; u < groupByKeys.length; u++) {
...
function getDataByRadioButton(selectedRadioBtnValue, groupby) {
    let groupedProductList = createGroupedProductsList(ProductList, groupby);
    let groupValues = Object.assign({}, groupedProductList);
    let updatedTbl = generateTable(groupValues[selectedRadioBtnValue]);
    return updatedTbl;
}

function getSearchedTextResults(searchedText) {
    let searchText = searchedText.toLowerCase();
    let filteredProductList = ProductList.filter((n, i) => {
        return n.categoryName.toLowerCase().includes(searchText) || 
            n.productName.toLowerCase().includes(searchText) || 
            n.manufacturer.toLowerCase().includes(searchText);
    });

    let updatedtbl = generateTable(filteredProductList);
    return updatedtbl;
}